(function ($) {
    ...
    // Función para avanzar al siguiente elemento del carrusel hacia la derecha
    function avanzarCarruselDerecha() {
        const headerCarousel = document.getElementById("header-carousel");
        const activeItem = headerCarousel.querySelector(".carousel-item.active");
    // Obtener el elemento anterior o el último elemento si no hay un elemento anterior
        const prevItem = activeItem.previousElementSibling || headerCarousel.querySelector(".carousel-item:last-child");
        activeItem.classList.remove("active");
        prevItem.classList.add("active");
}

// Cambiar automáticamente al siguiente elemento cada 5 segundos
setInterval(avanzarCarruselDerecha, 5000);
  ...
  const textos = ["Tecnología", "Desarrollo", "Innovación"]; // Array de textos a mostrar
